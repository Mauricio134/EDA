...
public:
   Octree(Point , double );
   bool exist( const Point & , Node * &, double &, Node *&);
...
bool Octree::exist(const Point &p, Node * &n, double &pos, Node *& k){
    int child;
    Node * pp = n;
    if(p.x < root->getBL().x || p.x > root->getH() || p.y < root->getBL().y || p.y > root->getH() || p.z < root->getBL().z || p.z > root->getH()){
        return 1;
    }
    ...
    double midx = (2*n->getBL().x + n->getH())/2;
    double midy = (2*n->getBL().y + n->getH())/2;
    double midz = (2*n->getBL().z + n->getH())/2;
